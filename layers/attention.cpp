...
xt::xarray<float> Attention::feedforward(const xt::xarray<float>& inputs, Mode mode) {
    ...
    xt::xtensor<float, 3> Q = xt::eval(xt::reshape_view(fast_dot(E, weights_q), {batch_size * num_heads, seq_len, d_k}));
    xt::xtensor<float, 3> K = xt::eval(xt::reshape_view(fast_dot(E, weights_k), {batch_size * num_heads, seq_len, d_k}));
    xt::xtensor<float, 3> V = xt::eval(xt::reshape_view(fast_dot(E, weights_v), {batch_size * num_heads, seq_len, d_k}));
    ...
    for (size_t batch = 0; batch < batch_size; batch++) {
        for (size_t head = 0; head < num_heads; head++) {
            ...
            Ri[index] = fast_dot(Qi[index], xt::eval(xt::transpose(Ki[index]))) / sqrt(d_k) + mask;
            Ai[index] = softmax(Ri[index]);

            xt::xtensor<float, 2> Ci = fast_dot(Ai[index], Vi[index]);
            set_3d(C, Ci, index);
        }
    }
    ...
    outputs = fast_dot(C_reshaped, weights_o);
...
xt::xarray<float> Attention::backprop(const xt::xarray<float>& p_delta, bool calc_delta_activation) {
    ...
    xt::xtensor<float, 2> delta_C = fast_dot(delta, xt::eval(xt::transpose(weights_o)));
    xt::xtensor<float, 3> delta_C_reshaped = xt::eval(xt::reshape_view(delta_C, {batch_size * num_heads, seq_len, d_k}));

    grad_weights_o += fast_dot(xt::eval(xt::transpose(C_reshaped)), delta);
    ...
    for (size_t batch = 0; batch < batch_size; batch++) {
        for (size_t head = 0; head < num_heads; head++) {
            ...
            xt::xtensor<float, 2> delta_Vi = fast_dot(xt::eval(xt::transpose(Ai[index])), delta_Ci);
            xt::xtensor<float, 2> delta_Ai = fast_dot(delta_Ci, xt::eval(xt::transpose(Vi[index])));
            ...
            xt::xtensor<float, 2> delta_Qi = fast_dot(delta_Ri, Ki[index]) / sqrt(d_k);
            xt::xtensor<float, 2> delta_Ki = fast_dot(xt::eval(xt::transpose(delta_Ri)), Qi[index]) / sqrt(d_k);
    ...
    grad_weights_q += fast_dot(E_T, delta_Q_reshaped);
    grad_weights_k += fast_dot(E_T, delta_K_reshaped);
    grad_weights_v += fast_dot(E_T, delta_V_reshaped);

    xt::xtensor<float, 2> delta_E = fast_dot(delta_Q_reshaped, xt::eval(xt::transpose(weights_q))) +
                                    fast_dot(delta_K_reshaped, xt::eval(xt::transpose(weights_k))) +
                                    fast_dot(delta_V_reshaped, xt::eval(xt::transpose(weights_v)));
