...
xt::xarray<float> Attention::feedforward(const xt::xarray<float>& inputs, Mode mode) {
    ...
    xt::xtensor<float, 3> Q = xt::eval(xt::reshape_view(eigen_dot(E, weights_q), {batch_size * num_heads, seq_len, d_k}));
    xt::xtensor<float, 3> K = xt::eval(xt::reshape_view(eigen_dot(E, weights_k), {batch_size * num_heads, seq_len, d_k}));
    xt::xtensor<float, 3> V = xt::eval(xt::reshape_view(eigen_dot(E, weights_v), {batch_size * num_heads, seq_len, d_k}));
    ...
    for (size_t batch = 0; batch < batch_size; batch++) {
        for (size_t head = 0; head < num_heads; head++) {
            ...
            Ri[index] = eigen_dot(Qi[index], xt::eval(xt::transpose(Ki[index]))) / sqrt(d_k) + mask;
            Ai[index] = softmax(Ri[index]);

            xt::xtensor<float, 2> Ci = eigen_dot(Ai[index], Vi[index]);
            set_3d(C, Ci, index);
        }
    }
    ...
    outputs = eigen_dot(C_reshaped, weights_o);
...
xt::xarray<float> Attention::backprop(const xt::xarray<float>& p_delta, bool calc_delta_activation) {
    ...
    xt::xtensor<float, 2> delta_C = eigen_dot(delta, xt::eval(xt::transpose(weights_o)));
    xt::xtensor<float, 3> delta_C_reshaped = xt::eval(xt::reshape_view(delta_C, {batch_size * num_heads, seq_len, d_k}));

    grad_weights_o += eigen_dot(xt::eval(xt::transpose(C_reshaped)), delta);
    ...
    for (size_t batch = 0; batch < batch_size; batch++) {
        for (size_t head = 0; head < num_heads; head++) {
            ...
            xt::xtensor<float, 2> delta_Vi = eigen_dot(xt::eval(xt::transpose(Ai[index])), delta_Ci);
            xt::xtensor<float, 2> delta_Ai = eigen_dot(delta_Ci, xt::eval(xt::transpose(Vi[index])));
            ...
            xt::xtensor<float, 2> delta_Qi = eigen_dot(delta_Ri, Ki[index]) / sqrt(d_k);
            xt::xtensor<float, 2> delta_Ki = eigen_dot(xt::eval(xt::transpose(delta_Ri)), Qi[index]) / sqrt(d_k);
    ...
    grad_weights_q += eigen_dot(E_T, delta_Q_reshaped);
    grad_weights_k += eigen_dot(E_T, delta_K_reshaped);
    grad_weights_v += eigen_dot(E_T, delta_V_reshaped);

    xt::xtensor<float, 2> delta_E = eigen_dot(delta_Q_reshaped, xt::eval(xt::transpose(weights_q))) +
                                    eigen_dot(delta_K_reshaped, xt::eval(xt::transpose(weights_k))) +
                                    eigen_dot(delta_V_reshaped, xt::eval(xt::transpose(weights_v)));
